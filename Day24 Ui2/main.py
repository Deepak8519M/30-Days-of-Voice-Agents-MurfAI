...
async def stream_gemini_response(transcript: str, websocket: WebSocket) -> Optional[str]:
    ...
    try:
        ...
        murf_ws_url = f"{MURF_WS_URL}?api_key={MURF_API_KEY}&context_id={CONTEXT_ID}&format=PCM&sample_rate=44100&channel_type=MONO"
    ...
    async def forward_event(client, message):
        ...
        try:
            if message.type == "Turn" and message.transcript:
                ...
                await websocket.send_json({"type": "transcription", "data": transcript_text})  # Send as JSON
                if hasattr(message, "turn_is_formatted") and message.turn_is_formatted:
                    ...
                    await websocket.send_json({"type": "transcription", "data": final_transcript})  # Send final as JSON
