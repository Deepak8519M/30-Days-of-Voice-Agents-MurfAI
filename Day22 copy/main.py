...
async def stream_gemini_response(transcript: str, websocket: WebSocket):
    ...
    try:
        ...
        text_buffer = ""  # Buffer to collect text chunks
        ...
        async with websockets.connect(murf_ws_url) as murf_ws:
            # Initial connection message with voice configuration
            init_message = {
                "init": True,
                "voice_config": {
            ...
            await murf_ws.send(json.dumps(init_message))
            log.debug(f"Sent init message to Murf: {init_message}")
            ...
            for chunk in response:
                if chunk.text:
                    ...
                    text_buffer += content + " "
                    # Send to Murf only if we have a complete sentence (ends with .!?)
                    if content.endswith(('.', '!', '?')):
                        log.debug(f"Sending buffered text to Murf: {text_buffer}")
                        await murf_ws.send(json.dumps({"text": text_buffer}))
                        text_buffer = ""  # Reset buffer
                        # Receive audio chunks with timeout
                        start_time = time.time()
                        while time.time() - start_time < 15:  # 15-second timeout per chunk
                            try:
                                murf_response = await asyncio.wait_for(murf_ws.recv(), timeout=2.0)
                                log.debug(f"Received from Murf: {murf_response[:100]}...")
                                ...
                                log.debug("Timeout waiting for Murf response, continuing...")
                                continue
                        if is_final:
                            break
            ...
            if text_buffer.strip():
                log.debug(f"Sending remaining buffered text to Murf: {text_buffer}")
                await murf_ws.send(json.dumps({"text": text_buffer}))
                ...
                while time.time() - start_time < 15:
                    try:
                        ...
                        log.debug("Timeout waiting for final Murf response, assuming complete")
                        break
