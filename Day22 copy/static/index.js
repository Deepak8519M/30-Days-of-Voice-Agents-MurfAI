...
const BUFFER_GAP = 0.005; // 5ms gap to prevent clicks
...
function initAudioContext() {
  if (!audioContext) {
    ...
    gainNode.gain.setValueAtTime(0.8, audioContext.currentTime); // Normalize volume
...
async function queueAudio(base64Audio, isFinal) {
  try {
    ...
    for (let i = 0; i < int16.length; i++) {
      float32[i] = int16[i] / 32768; // Convert to float32
    }
